...
Seq(
  Customer(1, "old address for 1", false, null, "2018-02-01", sha256(concat(address, marital_status, date_of_birth))),
...
// MAGIC WHEN MATCHED AND customers.current = true AND (customers.address <> staged_updates.address OR source.attr2 <> target.attr2 OR source.attr3 <> target.attr3) THEN  
...
1900-01-01 - default start_date
9999-12-31 - default end_date
